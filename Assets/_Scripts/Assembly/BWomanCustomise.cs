...
public class BWomanCustomise : MonoBehaviour {
	...
	public void charCustomize (int face, int eye, int topT, int bottomT, int hairT, int hairC, int skinT, int jacketC, int shirtC, int skirtC, int pantsC, int shoesC, int glasses)
	{
		...
		for (int i = 0; i < materialsList.faceType.Length; i++) {
            ...
            materialsList.glassesType[i].SetActive(false);
        }
		materialsList.faceType [face].SetActive (true);

		if(glasses > 0)
		{
            materialsList.glassesType[face].SetActive(true);

            foreach (Transform child in materialsList.glassesType[face].transform)
            {
                ...
                mat = new Material[2];
                mat[0] = materialsList.GlassesFrameMaterial[0];
                mat[1] = materialsList.GlassesMaterials[glasses-1];
	...
	void OnValidate ()
	{
        ...
        charCustomize (faceTyp, eyeCol, topTyp, btmTyp, hairTyp, hairCol, skinTyp, jacketCol, shirtCol, skirtCol, pantsCol, shoesCol, glassesTyp);
